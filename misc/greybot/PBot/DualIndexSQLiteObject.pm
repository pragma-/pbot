...
sub begin {
    ...
    print("Opening $self->{name} database ($self->{filename})\n");
    ...
    if ($@) { print("Error initializing $self->{name} database: $@\n"); }
}

sub end {
    my ($self) = @_;

    print("Closing $self->{name} database ($self->{filename})\n");
...
sub create_cache {
    ...
    print("Cached $index2_count $self->{name} objects in $index1_count groups.\n");
}
...
sub get_data {
    ...
    if (not defined $data_index) {
        ...
        if ($@) {
            print("Error getting data for ($index1, $index2): $@\n");
            return undef;
        }
    ...
    if ($@) {
        print("Error getting data for ($index1, $index2, $data_index): $@\n");
        return undef;
    }
...
sub add {
    ...
    if ($@) {
        print("Error adding $self->{name} $index2 to $index1: $@\n");
        $self->{dbh}->rollback;
    ...
    print("$self->{name}: [$index1]: $index2 added.\n") unless $quiet;
...
sub remove {
    ...
    if (not defined $index2) {
        ...
        if ($@) {
            print("Error removing $index1 from $self->{name}: $@\n");
    ...
    if (not defined $data_index) {
        ...
        if ($@) {
            print("Error removing $self->{name}: [$name1] $name2: $@\n");
    ...
    if (defined $self->get_data($lc_index1, $lc_index2, $data_index)) {
        ...
        if ($@) {
            print("Error unsetting $self->{name}: $name1.$name2: $@\n");
...
sub set {
    ...
    if (not defined $value) { $value = $self->get_data($index1, $index2, $key); }
        ...
        if ($@) {
            print("Error setting $self->{name} $index1 $index2.$key: $@\n");
