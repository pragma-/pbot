...
package lib4422::DualIndexSQLiteObject;
...
sub load_metadata {
    ...
    eval {
        my %existing = ();
        foreach my $col (@{$self->{dbh}->selectall_arrayref("PRAGMA TABLE_INFO(Stuff)")}) {
            $self->{columns}->{$col->[1]} = $col->[2];
        }
    };

    if ($@) {
        $self->{pbot}->{logger}->log("Error loading metadata for $self->{name}: $@");
    }
}
...
sub set {
    my ($self, $index1, $index2, $key, $value, $dont_save) = @_;
